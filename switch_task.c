...
extern xQueueHandle g_pWatchQueue;
		...
		if (ui8CurButtonState != ui8PrevButtonState) {
			...
			if ((ui8CurButtonState & ALL_BUTTONS) != 0) {
				if ((ui8CurButtonState & ALL_BUTTONS) == LEFT_BUTTON) {
					...
					//clear the screen and return to (0.0)
					clearLCD();
					printLCD("Your Heart Rate:");
					if (HeartRateTaskInit() != 0) {
						while (1) {
						}
					}
					...
					xSemaphoreGive(g_pUARTSemaphore);
					xTimerStop(xAutoReloadTimer1, 0);

					//delete heart rate task
				...
				//
				if (xQueueSend(g_pWatchQueue, &ui8Message, portMAX_DELAY) !=
				pdPASS) {
					//
